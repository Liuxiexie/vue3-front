...
<template>
  <div class="bg-white dark:bg-zinc-900 sticky top-0 left-0 z-10">
    <ul ref="ulTarget" class="relative flex overflow-x-auto p-1 text-xs text-zinc-600">
      <!-- 滑块 -->
      <li :style="sliderStyle" ref="sliderTarget"
        class="absolute bg-zinc-900 dark:bg-zinc-800 h-[22px] rounded-lg duration-200"></li>
      <!-- 汉堡按钮 -->
      <li @click="onShowPopup"
        class="fixed top-0 right-[-1px] h-4 p-1 flex items-center bg-white dark:bg-zinc-900 z-20 shadow-l-white dark:shadow-l-zinc">
