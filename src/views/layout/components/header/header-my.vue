...
<template>
  <m-popover placement="bottom-left" class="flex items-center ">
    <template #reference>
      <div
        class="relative flex items-center p-0.5 rounded-sm cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-900 duration-200 outline-none">
        <!-- 头像 -->
        <img class="w-3 h-3 rounded-sm"
          ...
          alt="">
        <!-- 下拉箭头 -->
        <m-svg-icon name="down-arrow" class="h-1.5 w-1.5 ml-0" fillClass="fill-zinc-900 dark:fill-zinc-300"></m-svg-icon>
        <!-- vip -->
    ...
    <div class="w-[140px] overflow-hidden">
      <div v-for="item in menuArr" :key="item.id"
        class="flex items-center cursor-pointer p-1 rounded hover:bg-zinc-100 dark:hover:bg-zinc-800">
